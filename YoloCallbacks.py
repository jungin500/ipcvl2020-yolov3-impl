...
class VisualizeYolo(tf.keras.callbacks.Callback):
    def __init__(self, test_dataset, visualizer_fn, model, display_on_begin=False, display_freq_epoches=200, **kwargs):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch % self.display_freq_epoches == 0:
            ...
            batch_image_index = random.randrange(0, self.test_dataset.batch_size)
            print("\n[Visualizer] Pulling out batch index [%d/%d]" % (batch_range + 1, self.test_dataset.__len__()))
            print("[Visualizer] Displaying one image out of batch [%d/%d]" % (batch_image_index + 1, self.test_dataset.batch_size))
